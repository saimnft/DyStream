...
class InferenceCallback(Callback):
    ...
    def on_train_start(self, trainer, pl_module):
        ...
        if experiment is not None and hasattr(experiment, "define_metric"):
            experiment.define_metric("videos", step_metric="inference_steps")
    ...
    def on_train_batch_end(self, trainer, pl_module, outputs, batch, batch_idx):
        if trainer.is_global_zero and ((trainer.global_step % self.steps_interval == 0) or (pl_module.cfg.test_first and trainer.global_step == 1)):
            ...
            for key, save_video_dir_dya in infer_dict["saved_videos"].items():
                video_to_log = []
                if not os.path.exists(save_video_dir_dya):
                    continue
                ...
                for save_file in sorted(os.listdir(save_video_dir_dya)):
                    ...
                    if len(video_to_log) > 50:
                        break
                experiment = getattr(pl_module.logger, "experiment", None)
                if experiment is not None and hasattr(experiment, "log"):
                    experiment.log({f"videos_{key}": video_to_log, "inference_steps": trainer.global_step})
...
class MotionGenLightningModule(LightningModule):
    ...
    def _step(self, batch, is_training=True):
        ...
        mask_noise_latent = motion_latent
        

        # linear_weights = torch.arange(1, t + 1, dtype=torch.float32, device=motion_latent.device)

        # random_indices = torch.multinomial(linear_weights, num_samples=bs, replacement=True)
    ...
    def _inference_one_file(self, test_file, test_loss, total_length, ref_img_path, save_path, calculate_loss=False):
        ...
        np.savez(
            ...
            video_id=test_file["video_id"] + f"_cfg_fusion_{self.cfg.model.cfg_fusion}_cfg_prev_motion_{self.cfg.model.cfg_prev_motion}_cfg_anchor_{self.cfg.model.cfg_anchor}")
    ...
    def _get_latent(self, test_file):
        ...
        if not os.path.exists(test_file["motion_self_path"]):
            ...
            # Step 1: ensure ori_img_path exists
            if not os.path.exists(ori_img_path) and os.path.exists(video_path):
                ...
                if ret:
                    ...
                    return None
            # Step 2: ensure masked_img_path exists
            print(masked_img_path, ori_img_path)
            if not os.path.exists(masked_img_path) and os.path.exists(ori_img_path):
                cmd_mask = f"cd {self.cfg.tools_path}/tools/visualization_0416 && python img_to_mask.py --image_path {ori_img_path} --save_path {ori_img_path} --crop True --union_bbox_scale 1.6 && cd {current_path}"
                rank_zero_info(f"Running command: {cmd_mask}")
                os.system(cmd_mask)

            # Step 3: ensure latent_path exists
            ori_img_path = ori_img_path.replace(".png", "_resize.png")
            cmd_latent = f"cd {self.cfg.tools_path}/tools/visualization_0416 && python img_to_latent.py --mask_image_path {ori_img_path} --save_npz_path {latent_path} --version {self.cfg.model.version} && cd {current_path}"
    ...
    def inference_step(self, test_path, save_path, **kwargs):
        with self.ema.average_parameters(self.model.parameters()):
            ...
            all_test_list = []
            for data_meta_path in test_path:
                test_list, infp_save_path, infp_save_video_dir, test_loss, total_length = self._get_test_list(data_meta_path, "infp_demo", save_path, select_range=(0, 4), is_test=True)
                all_test_list.extend(test_list)
            test_list = all_test_list
            ...
            if len(test_list) > 0:
                ...
                self._render_and_evaluate(infp_save_path, infp_save_video_dir, metrics, "infp_demo")
                save_videos["infp_demo"] = infp_save_video_dir
...
def main():
    ...
    if cfg.get("resume_model_only", False):
        init_ckpt = cfg.get("init_ckpt", None) or cfg.resume_ckpt
        fit_ckpt_path = None
        if init_ckpt:
            rank_zero_info(f"Loading model weights only from {init_ckpt}")
            checkpoint = torch.load(init_ckpt, map_location="cpu")
            state_dict = checkpoint.get("state_dict", checkpoint)
            missing_keys, unexpected_keys = model.load_state_dict(state_dict, strict=False)
            rank_zero_info(f"Loaded model-only checkpoint: missing_keys={len(missing_keys)}, unexpected_keys={len(unexpected_keys)}")
            if "ema_state" in checkpoint:
                model.ema.load_state_dict(checkpoint["ema_state"])
                rank_zero_info("Loaded EMA state from model-only checkpoint")
    ...
    if not cfg.is_test: 
        ...
        trainer.fit(model, train_dataloader, val_dataloaders=test_dataloader, ckpt_path=fit_ckpt_path)
